...
export default createStore({
  ...
  actions: {
    ...
    async fetchMessages({ commit, state }) {
      ...
      try {
        const response = await axios.post('http://206.189.129.58/history', { session_id: state.sessionId });
    ...
    async sendMessage({ commit, state }, messageContent) {
      ...
      try {
        const response = await axios.post('http://206.189.129.58/chat', { input: messageContent, session_id: state.sessionId });
