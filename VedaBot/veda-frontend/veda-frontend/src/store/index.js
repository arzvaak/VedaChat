...
export default createStore({
  ...
  actions: {
    ...
    async fetchMessages({ commit, state }) {
      ...
      try {
        const response = await axios.post('http://localhost:5000/history', { session_id: state.sessionId });
    ...
    async sendMessage({ commit, state }, messageContent) {
      ...
      try {
        const response = await axios.post('http://localhost:5000/chat', { input: messageContent, session_id: state.sessionId });
